...
use super::types::{Entry, Field, Group, MemoryProtection, Meta, Times, XmlDatabase, Value};
use std::io::Read;
use thiserror::Error;
use uuid::Uuid;
    ...
    loop {
        match xml_event_reader.next()? {
            XmlEvent::Characters(chardata) => {
                content = content.map_or_else(|| Some(chardata.clone()), |mut existing: String| {
...
fn parse_field<R: Read>(xml_event_reader: &mut EventReader<R>) -> Result<Field> {
    let mut field = Field::default();
    loop {
        match xml_event_reader.next()? {
            XmlEvent::StartElement { name, .. } if &name.local_name == "Key" => {
                field.key = parse_string(xml_event_reader).and_then(|val| val.ok_or(Error::KeyEmptyName))?;
            },
            XmlEvent::StartElement { name, attributes, .. } if &name.local_name == "Value" => {
                let protected = attributes.iter()
                    .any(|attr| attr.name.local_name == "Protected" && attr.value.to_lowercase() == "true");
            ...
            XmlEvent::EndElement { name, .. } if &name.local_name == "String" => {
                break
            },
            _ => {}
        }
...
fn parse_history<R: Read>(xml_event_reader: &mut EventReader<R>) -> Result<Vec<Entry>> {
    ...
    loop {
        match xml_event_reader.next()? {
            ...
            XmlEvent::EndElement { name, .. } if &name.local_name == "History" => {
                break
            },
            _ => {}
        }
        ...
        match xml_event_reader.next()? {
            XmlEvent::StartElement { name, .. } => {
                if &name.local_name == "LastModificationTime" {
                    ...
                    times.usage_count = parse_u32(xml_event_reader)?
                        .unwrap_or_default();
                }
            }
            XmlEvent::EndElement { name, .. } if &name.local_name == "Times" => {
                break
            },
            _ => {}
        }
        ...
        match xml_event_reader.next()? {
            XmlEvent::StartElement { name, .. } => {
                if &name.local_name == "History" {
                    ...
                    entry.fields.push(parse_field(xml_event_reader)?);
            ...
            XmlEvent::EndElement { name, .. } if &name.local_name == "Entry" => {
                break
            },
            _ => {}
        }
...
fn parse_root<R: Read>(xml_event_reader: &mut EventReader<R>) -> Result<Vec<Group>> {
    ...
    loop {
        match xml_event_reader.next()? {
            ...
            XmlEvent::EndElement { name, .. } if &name.local_name == "Root" => {
                break
            },
            _ => {}
        }
        ...
        match xml_event_reader.next()? {
            XmlEvent::StartElement { name, .. } => {
                match name.local_name.as_ref() {
                    "String" => {
                        fields.push(parse_field(xml_event_reader)?);
                    },
                    _ => {}
                }
            },            
...
fn parse_memory_protection<R: Read>(xml_event_reader: &mut EventReader<R>) -> Result<MemoryProtection> {
    ...
    loop {
        match xml_event_reader.next()? {
            ...
            XmlEvent::EndElement { name, .. } if &name.local_name == "MemoryProtection" => {
                break
            },
            _ => {}
        }
...
fn parse_meta<R: Read>(xml_event_reader: &mut EventReader<R>) -> Result<Meta> {
    ...
    loop {
        match xml_event_reader.next()? {
            ...
            XmlEvent::EndElement { name, .. } if &name.local_name == "Meta" => {
                break
            },
            _ => {}
        }
...
fn parse_file<R: Read>(xml_event_reader: &mut EventReader<R>) -> Result<XmlDatabase> {
    let mut db = XmlDatabase::default();
    loop {
        match xml_event_reader.next()? {
            ...
            XmlEvent::EndElement { name, .. } if &name.local_name == "KeePassFile" => {
                break
            },
            _ => {}
        }
